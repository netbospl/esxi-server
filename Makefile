...
.PHONY: check check-local check-ci bash shellcheck contracts evals inventory packer-contract markdown xml yaml packer tests secrets ignores links actionlint require-tool

check: check-local
check-local: bash shellcheck contracts tests xml yaml packer markdown secrets ignores links actionlint
